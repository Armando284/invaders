const OUTPUT_GAIN = 0.07

const MUSIC_STEP = 0.16
const TICK_INTERVAL = 25

// Original 16-step chiptune loop, distinct from the MAZE theme: a driving
// eighth-note bass against syncopated lead punches off the beat.
const MELODY = [
	0, 659.25, 440, 0, 523.25, 659.25, 0, 440,
	0, 587.33, 440, 0, 523.25, 0, 493.88, 0,
]
const BASS = [
	110, 110, 0, 110, 110, 0, 110, 110,
	98, 98, 0, 98, 87.31, 87.31, 0, 110,
]

export class Sfx {
	private context: AudioContext | null = null
	private output: GainNode | null = null
	private muted = false
	private musicPlaying = false
	private musicStep = 0
	private nextNoteTime = 0
	private ticker = 0
	private tempo = 1

	get isMuted(): boolean {
		return this.muted
	}

	setMuted(muted: boolean): void {
		this.muted = muted
	}

	setTempo(factor: number): void {
		this.tempo = factor
	}

	unlock(): void {
		this.ensure()
	}

	startMusic(): void {
		this.ensure()

		if (!this.context) {
			return
		}

		this.musicPlaying = true
		this.musicStep = 0
		this.nextNoteTime = this.context.currentTime + 0.05

		if (this.ticker === 0) {
			this.ticker = window.setInterval(() => this.tick(), TICK_INTERVAL)
		}
	}

	pauseMusic(): void {
		this.musicPlaying = false
	}

	resumeMusic(): void {
		this.musicPlaying = true
	}

	stopMusic(): void {
		this.musicPlaying = false

		if (this.ticker > 0) {
			window.clearInterval(this.ticker)
			this.ticker = 0
		}
	}

	private tick(): void {
		const context = this.context

		if (!context || !this.musicPlaying) {
			return
		}

		if (this.nextNoteTime < context.currentTime - 0.1) {
			this.nextNoteTime = context.currentTime + 0.05
		}

		while (this.nextNoteTime < context.currentTime + 0.12) {
			this.playStep(this.musicStep, this.nextNoteTime)
			this.musicStep = (this.musicStep + 1) % MELODY.length
			this.nextNoteTime += MUSIC_STEP * this.tempo
		}
	}

	private playStep(step: number, startTime: number): void {
		const melody = MELODY[step] ?? 0
		const bass = BASS[step] ?? 0

		if (melody) {
			this.emit(melody, startTime, 0.1)
		}

		if (bass) {
			this.emit(bass, startTime, 0.11)
		}
	}

	start(): void {
		this.tone(440, 0.08)
		this.tone(660, 0.09, 0.07)
	}

	fire(): void {
		this.tone(820, 0.05)
		this.tone(520, 0.05, 0.04)
	}

	alienFire(): void {
		this.tone(300, 0.08)
		this.tone(240, 0.06, 0.07)
	}

	hit(chain = 1): void {
		const pitch = 1 + (chain - 1) * 0.1
		this.tone(720 * pitch, 0.05)
		this.tone(440 * pitch * 1.12, 0.09, 0.05)
	}

	ufo(): void {
		this.tone(520, 0.06)
		this.tone(780, 0.06, 0.05)
		this.tone(1040, 0.09, 0.1)
	}

	shield(): void {
		this.tone(150, 0.07)
	}

	loseLife(): void {
		this.tone(320, 0.16)
		this.tone(200, 0.18, 0.14)
		this.tone(120, 0.32, 0.3)
	}

	extra(): void {
		const notes = [440, 554, 659, 880]
		notes.forEach((note, index) => {
			this.tone(note, 0.09, index * 0.08)
		})
	}

	wave(): void {
		const notes = [523.25, 659.25, 783.99, 1046.5]
		notes.forEach((note, index) => {
			this.tone(note, 0.12, index * 0.1)
		})
	}

	victory(): void {
		const notes = [392, 523.25, 659.25, 783.99, 1046.5, 1318.51]
		notes.forEach((note, index) => {
			this.tone(note, 0.14, index * 0.12)
		})
	}

	gameOver(): void {
		const notes = [300, 250, 200, 150]
		notes.forEach((note, index) => {
			this.tone(note, 0.24, index * 0.18)
		})
	}

	private ensure(): AudioContext | null {
		if (typeof window === 'undefined') {
			return null
		}

		if (!this.context) {
			const AudioContextCtor =
				window.AudioContext ??
				(
					window as unknown as {
						webkitAudioContext?: typeof AudioContext
					}
				).webkitAudioContext

			if (!AudioContextCtor) {
				return null
			}

			this.context = new AudioContextCtor()
			this.output = this.context.createGain()
			this.output.gain.value = OUTPUT_GAIN
			this.output.connect(this.context.destination)
		}

		if (this.context.state === 'suspended') {
			this.context.resume()
		}

		return this.context
	}

	private tone(frequency: number, duration: number, delay = 0): void {
		const context = this.ensure()

		if (!context) {
			return
		}

		this.emit(frequency, context.currentTime + delay, duration)
	}

	private emit(frequency: number, startTime: number, duration: number): void {
		if (this.muted) {
			return
		}

		const context = this.context
		const output = this.output

		if (!context || !output) {
			return
		}

		const oscillator = context.createOscillator()
		const gain = context.createGain()

		oscillator.type = 'square'
		oscillator.frequency.setValueAtTime(frequency, startTime)

		gain.gain.setValueAtTime(1, startTime)
		gain.gain.exponentialRampToValueAtTime(0.001, startTime + duration)

		oscillator.connect(gain)
		gain.connect(output)

		oscillator.start(startTime)
		oscillator.stop(startTime + duration + 0.02)
	}
}